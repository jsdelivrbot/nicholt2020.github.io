...
(function(process) {
  !function(global, factory) {
    ...
    function _expandDefaultForm(ast, errors) {
      var children = ast.cases.map(function(c) {
        var expansionResult = expandNodes(c.expression);
        return errors.push.apply(errors, expansionResult.errors), "other" === c.value ? new Element("template", [new Attribute$1("ngSwitchDefault", "", c.valueSourceSpan)], expansionResult.nodes, c.sourceSpan, c.sourceSpan, c.sourceSpan) : new Element("template", [new Attribute$1("ngSwitchCase", "" + c.value, c.valueSourceSpan)], expansionResult.nodes, c.sourceSpan, c.sourceSpan, c.sourceSpan);
      }),
    ...
    function createInlineArray(values) {
      ...
      var log2 = Math.log(values.length) / Math.log(2),
          index = Math.ceil(log2),
          identifierSpec = index < Identifiers.inlineArrays.length ? Identifiers.inlineArrays[index] : Identifiers.InlineArrayDynamic,
          identifier = resolveIdentifier(identifierSpec);
    ...
    function createCheckBindingField(builder) {
      var bindingId = "" + builder.fields.length,
          fieldExpr = createBindFieldExpr(bindingId);
      return builder.fields.push(new ClassField(fieldExpr.name, null, [StmtModifier.Private])), builder.ctorStmts.push(THIS_EXPR.prop(fieldExpr.name).set(importExpr(resolveIdentifier(Identifiers.UNINITIALIZED))).toStmt()), new CheckBindingField(fieldExpr, bindingId);
    }
    function createCheckBindingStmt(evalResult, fieldExpr, throwOnChangeVar, actions) {
      ...
      return evalResult.forceUpdate && (condition = evalResult.forceUpdate.or(condition)), evalResult.stmts.concat([new IfStmt(condition, actions.concat([THIS_EXPR.prop(fieldExpr.name).set(evalResult.currValExpr).toStmt()]))]);
    }
    ...
    function convertPropertyBinding(builder, nameResolver, implicitReceiver, expression, bindingId) {
      var currValExpr = createCurrValueExpr(bindingId),
          stmts = [];
      nameResolver || (nameResolver = new DefaultNameResolver);
      var visitor = new _AstToIrVisitor(builder, nameResolver, implicitReceiver, VAL_UNWRAPPER_VAR, bindingId, (!1)),
          outputExpr = expression.visit(visitor, _Mode.Expression);
      if (!outputExpr)
        return null;
      ...
      return stmts.push(currValExpr.set(outputExpr).toDeclStmt(null, [StmtModifier.Final])), visitor.needsValueUnwrapper ? new ConvertPropertyBindingResult(stmts, currValExpr, VAL_UNWRAPPER_VAR.prop("hasWrappedValue")) : new ConvertPropertyBindingResult(stmts, currValExpr, null);
    }
    function convertActionBinding(builder, nameResolver, implicitReceiver, action, bindingId) {
      nameResolver || (nameResolver = new DefaultNameResolver);
      var visitor = new _AstToIrVisitor(builder, nameResolver, implicitReceiver, null, bindingId, (!0)),
          actionStmts = [];
      flattenStatements(action.visit(visitor, _Mode.Statement), actionStmts), prependTemporaryDecls(visitor.temporaryCount, bindingId, actionStmts);
    ...
    function createSharedBindingVariablesIfNeeded(stmts) {
      ...
      return readVars.has(VAL_UNWRAPPER_VAR.name) && unwrapperStmts.push(VAL_UNWRAPPER_VAR.set(importExpr(resolveIdentifier(Identifiers.ValueUnwrapper)).instantiate([])).toDeclStmt(null, [StmtModifier.Final])), unwrapperStmts;
    }
    ...
    function createCachedLiteralArray(builder, values) {
      ...
      for (var proxyExpr = THIS_EXPR.prop("_arr_" + builder.fields.length),
          proxyParams = [],
          proxyReturnEntries = [],
      ...
      return createPureProxy(fn(proxyParams, [new ReturnStatement(literalArr(proxyReturnEntries))], new ArrayType(DYNAMIC_TYPE)), values.length, proxyExpr, builder), proxyExpr.callFn(values);
    }
    function createCachedLiteralMap(builder, entries) {
      if (0 === entries.length)
        return importExpr(resolveIdentifier(Identifiers.EMPTY_MAP));
      for (var proxyExpr = THIS_EXPR.prop("_map_" + builder.fields.length),
          proxyParams = [],
          proxyReturnEntries = [],
          values = [],
      ...
      return createPureProxy(fn(proxyParams, [new ReturnStatement(literalMap(proxyReturnEntries))], new MapType(DYNAMIC_TYPE)), entries.length, proxyExpr, builder), proxyExpr.callFn(values);
    }
    ...
    function writeToRenderer(view, boundProp, renderElement, renderValue, logBindingUpdate, securityContextExpression) {
      var updateStmts = [],
          renderer = view.prop("renderer");
      switch (renderValue = sanitizedValue(view, boundProp, renderValue, securityContextExpression), boundProp.type) {
        case exports.PropertyBindingType.Property:
          logBindingUpdate && updateStmts.push(importExpr(resolveIdentifier(Identifiers.setBindingDebugInfo)).callFn([renderer, renderElement, literal(boundProp.name), renderValue]).toStmt()), updateStmts.push(renderer.callMethod("setElementProperty", [renderElement, literal(boundProp.name), renderValue]).toStmt());
          break;
        case exports.PropertyBindingType.Attribute:
          renderValue = renderValue.isBlank().conditional(NULL_EXPR, renderValue.callMethod("toString", [])), updateStmts.push(renderer.callMethod("setElementAttribute", [renderElement, literal(boundProp.name), renderValue]).toStmt());
          break;
        ...
        case exports.PropertyBindingType.Style:
          ...
          isPresent(boundProp.unit) && (strValue = strValue.plus(literal(boundProp.unit))), renderValue = renderValue.isBlank().conditional(NULL_EXPR, strValue), updateStmts.push(renderer.callMethod("setElementStyle", [renderElement, literal(boundProp.name), renderValue]).toStmt());
          break;
    ...
    function sanitizedValue(view, boundProp, renderValue, securityContextExpression) {
      if (boundProp.securityContext === _angular_core.SecurityContext.NONE)
        return renderValue;
      if (boundProp.needsRuntimeSecurityContext || (securityContextExpression = createEnumExpression(Identifiers.SecurityContext, boundProp.securityContext)), !securityContextExpression)
        throw new Error("internal error, no SecurityContext given " + boundProp.name);
      var ctx = view.prop("viewUtils").prop("sanitizer"),
          args = [securityContextExpression, renderValue];
      return ctx.callMethod("sanitize", args);
    }
    function triggerAnimation(view, componentView, boundProp, eventListener, renderElement, renderValue, lastRenderValue) {
      var detachStmts = [],
          updateStmts = [],
          animationName = boundProp.name,
          animationFnExpr = componentView.prop("componentType").prop("animations").key(literal(animationName)),
          emptyStateValue = literal(EMPTY_ANIMATION_STATE),
          unitializedValue = importExpr(resolveIdentifier(Identifiers.UNINITIALIZED)),
          animationTransitionVar = variable("animationTransition_" + animationName);
      updateStmts.push(animationTransitionVar.set(animationFnExpr.callFn([view, renderElement, lastRenderValue.equals(unitializedValue).conditional(emptyStateValue, lastRenderValue), renderValue.equals(unitializedValue).conditional(emptyStateValue, renderValue)])).toDeclStmt()), detachStmts.push(animationTransitionVar.set(animationFnExpr.callFn([view, renderElement, lastRenderValue, emptyStateValue])).toDeclStmt());
      ...
      return updateStmts.push.apply(updateStmts, registerStmts), detachStmts.push.apply(detachStmts, registerStmts), {
    ...
    function addCheckHostMethod(hostProps, builder) {
      var stmts = [],
          methodParams = [new FnParam(VIEW_VAR.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(COMPONENT_VIEW_VAR.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(RENDER_EL_VAR.name, DYNAMIC_TYPE), new FnParam(THROW_ON_CHANGE_VAR.name, BOOL_TYPE)];
      hostProps.forEach(function(hostProp, hostPropIdx) {
        var field = createCheckBindingField(builder),
            evalResult = convertPropertyBinding(builder, null, THIS_EXPR.prop(CONTEXT_FIELD_NAME), hostProp.value, field.bindingId);
        if (evalResult) {
          var securityContextExpr;
          hostProp.needsRuntimeSecurityContext && (securityContextExpr = variable("secCtx_" + methodParams.length), methodParams.push(new FnParam(securityContextExpr.name, importType(resolveIdentifier(Identifiers.SecurityContext)))));
          ...
          if (hostProp.isAnimation) {
            ...
            checkBindingStmts = updateStmts, (_b = builder.detachStmts).push.apply(_b, detachStmts);
          } else
    ...
    function addHandleEventMethod(hostListeners, builder) {
      var resultVar = variable("result"),
          actionStmts = [resultVar.set(literal(!0)).toDeclStmt(BOOL_TYPE)];
      hostListeners.forEach(function(hostListener, eventIdx) {
        var evalResult = convertActionBinding(builder, null, THIS_EXPR.prop(CONTEXT_FIELD_NAME), hostListener.handler, "sub_" + eventIdx),
            trueStmts = evalResult.stmts;
        evalResult.preventDefault && trueStmts.push(resultVar.set(evalResult.preventDefault.and(resultVar)).toStmt()), actionStmts.push(new IfStmt(EVENT_NAME_VAR.equals(literal(hostListener.fullName)), trueStmts));
      }), actionStmts.push(new ReturnStatement(resultVar)), builder.methods.push(new ClassMethod("handleEvent", [new FnParam(EVENT_NAME_VAR.name, STRING_TYPE), new FnParam(EventHandlerVars.event.name, DYNAMIC_TYPE)], actionStmts, BOOL_TYPE));
    }
    ...
    function parseHostBindings(dirMeta, exprParser, schemaRegistry) {
      var errors = [],
          parser = new BindingParser(exprParser, DEFAULT_INTERPOLATION_CONFIG, schemaRegistry, [], errors),
          sourceFileName = dirMeta.type.moduleUrl ? "in Directive " + dirMeta.type.name + " in " + dirMeta.type.moduleUrl : "in Directive " + dirMeta.type.name,
          ...
          parsedHostProps = parser.createDirectiveHostPropertyAsts(dirMeta.toSummary(), sourceSpan),
          parsedHostListeners = parser.createDirectiveHostEventAsts(dirMeta.toSummary(), sourceSpan);
    ...
    function reportParseErrors(parseErrors, console) {
      var warnings = parseErrors.filter(function(error) {
        return error.level === exports.ParseErrorLevel.WARNING;
          ...
          errors = parseErrors.filter(function(error) {
            return error.level === exports.ParseErrorLevel.FATAL;
          });
      if (warnings.length > 0 && this._console.warn("Directive parse warnings:\n" + warnings.join("\n")), errors.length > 0)
    ...
    function getPropertyInView(property, callingView, definedView) {
      ...
      for (var viewProp = THIS_EXPR,
        ...
        currView = currView.declarationElement.view, viewProp = viewProp.prop("parentView");
    ...
    function injectFromViewParentInjector(view, token, optional) {
      var viewExpr;
      viewExpr = view.viewType === ViewType.HOST ? THIS_EXPR : THIS_EXPR.prop("parentView");
      var args = [createDiTokenExpression(token), THIS_EXPR.prop("parentIndex")];
      return optional && args.push(NULL_EXPR), viewExpr.callMethod("injectorGet", args);
    }
    ...
    function createQueryValues(viewValues) {
      return ListWrapper.flatten(viewValues.values.map(function(entry) {
        return entry instanceof ViewQueryValues ? mapNestedViews(entry.view.declarationElement.viewContainer, entry.view, createQueryValues(entry)) : entry;
      }));
    }
    ...
    function bindOutputs(boundEvents, directives, compileElement, bindToRenderer) {
      ...
      return !!usedEvents.size && (bindToRenderer && subscribeToRenderEvents(usedEvents, compileElement), subscribeToDirectiveEvents(usedEvents, directives, compileElement), generateHandleEventMethod(boundEvents, directives, compileElement), !0);
    }
    function collectEvents(boundEvents, directives) {
      var usedEvents = new Map;
      return boundEvents.forEach(function(event) {
    ...
    function generateHandleEventMethod(boundEvents, directives, compileElement) {
      var hasComponentHostListener = directives.some(function(dirAst) {
        return dirAst.hostEvents.some(function(event) {
          ...
          markPathToRootStart = hasComponentHostListener ? compileElement.compViewExpr : THIS_EXPR,
          handleEventStmts = new CompileMethod(compileElement.view);
      handleEventStmts.resetDebugInfo(compileElement.nodeIndex, compileElement.sourceAst), handleEventStmts.push(markPathToRootStart.callMethod("markPathToRootAsCheckOnce", []).toStmt());
      var eventNameVar = variable("eventName"),
          resultVar = variable("result");
      handleEventStmts.push(resultVar.set(literal(!0)).toDeclStmt(BOOL_TYPE)), directives.forEach(function(dirAst, dirIdx) {
        var dirWrapper = compileElement.directiveWrapperInstance.get(dirAst.directive.type.reference);
        dirAst.hostEvents.length > 0 && handleEventStmts.push(resultVar.set(DirectiveWrapperExpressions.handleEvent(dirAst.hostEvents, dirWrapper, eventNameVar, EventHandlerVars.event).and(resultVar)).toStmt());
        ...
        var evalResult = convertActionBinding(compileElement.view, compileElement.view, compileElement.view.componentContext, renderEvent.handler, "sub_" + renderEventIdx),
            trueStmts = evalResult.stmts;
        evalResult.preventDefault && trueStmts.push(resultVar.set(evalResult.preventDefault.and(resultVar)).toStmt()), handleEventStmts.push(new IfStmt(eventNameVar.equals(literal(renderEvent.fullName)), trueStmts));
      }), handleEventStmts.push(new ReturnStatement(resultVar)), compileElement.view.methods.push(new ClassMethod(getHandleEventMethodName(compileElement.nodeIndex), [new FnParam(eventNameVar.name, STRING_TYPE), new FnParam(EventHandlerVars.event.name, DYNAMIC_TYPE)], handleEventStmts.finish(), BOOL_TYPE));
    }
    ...
    function bindDirectiveWrapperLifecycleCallbacks(dir, directiveWrapperIntance, compileElement) {
      compileElement.view.destroyMethod.addStmts(DirectiveWrapperExpressions.ngOnDestroy(dir.directive, directiveWrapperIntance)), compileElement.view.detachMethod.addStmts(DirectiveWrapperExpressions.ngOnDetach(dir.hostProperties, directiveWrapperIntance, THIS_EXPR, compileElement.compViewExpr || THIS_EXPR, compileElement.renderNode));
    }
    function bindInjectableDestroyLifecycleCallbacks(provider, providerInstance, compileElement) {
      var onDestroyMethod = compileElement.view.destroyMethod;
      onDestroyMethod.resetDebugInfo(compileElement.nodeIndex, compileElement.sourceAst), provider.providerType !== exports.ProviderAstType.Directive && provider.providerType !== exports.ProviderAstType.Component && provider.lifecycleHooks.indexOf(LifecycleHooks.OnDestroy) !== -1 && onDestroyMethod.addStmt(providerInstance.callMethod("ngOnDestroy", []).toStmt());
    }
    ...
    function bindRenderText(boundText, compileNode, view) {
      var valueField = createCheckBindingField(view),
          evalResult = convertPropertyBinding(view, view, view.componentContext, boundText.value, valueField.bindingId);
      return evalResult ? (view.detectChangesRenderPropertiesMethod.resetDebugInfo(compileNode.nodeIndex, boundText), void view.detectChangesRenderPropertiesMethod.addStmts(createCheckBindingStmt(evalResult, valueField.expression, DetectChangesVars.throwOnChange, [THIS_EXPR.prop("renderer").callMethod("setText", [compileNode.renderNode, evalResult.currValExpr]).toStmt()]))) : null;
    }
          ...
          switch (boundProp.type) {
            ...
            case exports.PropertyBindingType.Animation:
              ...
              var _a = triggerAnimation(THIS_EXPR, THIS_EXPR, boundProp, (hasEvents ? THIS_EXPR.prop(getHandleEventMethodName(compileElement.nodeIndex)) : importExpr(resolveIdentifier(Identifiers.noop))).callMethod(BuiltinMethod.Bind, [THIS_EXPR]), compileElement.renderNode, evalResult.currValExpr, bindingField.expression),
                  updateStmts = _a.updateStmts,
                  detachStmts = _a.detachStmts;
              checkBindingStmts.push.apply(checkBindingStmts, updateStmts), view.detachMethod.addStmts(detachStmts);
          }
    ...
    function bindDirectiveHostProps(directiveAst, directiveWrapperInstance, compileElement, elementName, schemaRegistry) {
      var runtimeSecurityCtxExprs = directiveAst.hostProperties.filter(function(boundProp) {
        ...
        switch (boundProp.type) {
          case exports.PropertyBindingType.Property:
            ctx = schemaRegistry.securityContext(elementName, boundProp.name, !1);
            break;
          case exports.PropertyBindingType.Attribute:
            ctx = schemaRegistry.securityContext(elementName, boundProp.name, !0);
            break;
          default:
    ...
    function bindDirectiveInputs(directiveAst, directiveWrapperInstance, dirIndex, compileElement) {
      ...
      detectChangesInInputsMethod.resetDebugInfo(compileElement.nodeIndex, compileElement.sourceAst), directiveAst.inputs.forEach(function(input, inputIdx) {
        ...
        evalResult && (detectChangesInInputsMethod.addStmts(evalResult.stmts), detectChangesInInputsMethod.addStmt(directiveWrapperInstance.callMethod("check_" + input.directiveName, [evalResult.currValExpr, DetectChangesVars.throwOnChange, evalResult.forceUpdate || literal(!1)]).toStmt()));
      });
      var isOnPushComp = directiveAst.directive.isComponent && !isDefaultChangeDetectionStrategy(directiveAst.directive.changeDetection),
          directiveDetectChangesExpr = DirectiveWrapperExpressions.ngDoCheck(directiveWrapperInstance, THIS_EXPR, compileElement.renderNode, DetectChangesVars.throwOnChange),
          directiveDetectChangesStmt = isOnPushComp ? new IfStmt(directiveDetectChangesExpr, [compileElement.compViewExpr.callMethod("markAsCheckOnce", []).toStmt()]) : directiveDetectChangesExpr.toStmt();
    ...
    function buildView(view, template, targetDependencies) {
      var builderVisitor = new ViewBuilderVisitor(view, targetDependencies),
          parentEl = view.declarationElement.isNull() ? view.declarationElement : view.declarationElement.parent;
      return templateVisitAll(builderVisitor, template, parentEl), view.viewType !== ViewType.EMBEDDED && view.viewType !== ViewType.HOST || (view.lastRenderNode = builderVisitor.getOrCreateLastRenderNode()), builderVisitor.nestedViewCount;
    }
    ...
    function _mergeHtmlAndDirectiveAttrs(declaredHtmlAttrs, directives) {
      ...
      var arrResult = [];
      return Object.keys(mapResult).sort().forEach(function(attrName) {
    ...
    function createViewClass(view, renderCompTypeVar, nodeDebugInfosVar) {
      var viewConstructorArgs = [new FnParam(ViewConstructorVars.viewUtils.name, importType(resolveIdentifier(Identifiers.ViewUtils))), new FnParam(ViewConstructorVars.parentView.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(ViewConstructorVars.parentIndex.name, NUMBER_TYPE), new FnParam(ViewConstructorVars.parentElement.name, DYNAMIC_TYPE)],
          superConstructorArgs = [variable(view.className), renderCompTypeVar, ViewTypeEnum.fromValue(view.viewType), ViewConstructorVars.viewUtils, ViewConstructorVars.parentView, ViewConstructorVars.parentIndex, ViewConstructorVars.parentElement, ChangeDetectorStatusEnum.fromValue(getChangeDetectionMode(view))];
      view.genConfig.genDebugInfo && superConstructorArgs.push(nodeDebugInfosVar), view.viewType === ViewType.EMBEDDED && (viewConstructorArgs.push(new FnParam("declaredViewContainer", importType(resolveIdentifier(Identifiers.ViewContainer)))), superConstructorArgs.push(variable("declaredViewContainer")));
    ...
    function generateDestroyMethod(view) {
      var stmts = [];
      return view.viewContainers.forEach(function(viewContainer) {
      ...
      }), stmts.push.apply(stmts, view.destroyMethod.finish()), stmts;
    }
    function generateCreateMethod(view) {
      var parentRenderNodeExpr = NULL_EXPR,
          parentRenderNodeStmts = [];
      view.viewType === ViewType.COMPONENT && (parentRenderNodeExpr = ViewProperties.renderer.callMethod("createViewRoot", [THIS_EXPR.prop("parentElement")]), parentRenderNodeStmts = [parentRenderNodeVar.set(parentRenderNodeExpr).toDeclStmt(importType(view.genConfig.renderTypes.renderNode), [StmtModifier.Final])]);
      var resultExpr;
    ...
    function generateDetectChangesMethod(view) {
      var stmts = [];
      if (view.animationBindingsMethod.isEmpty() && view.detectChangesInInputsMethod.isEmpty() && view.updateContentQueriesMethod.isEmpty() && view.afterContentLifecycleCallbacksMethod.isEmpty() && view.detectChangesRenderPropertiesMethod.isEmpty() && view.updateViewQueriesMethod.isEmpty() && view.afterViewLifecycleCallbacksMethod.isEmpty() && 0 === view.viewContainers.length && 0 === view.viewChildren.length)
        return stmts;
      stmts.push.apply(stmts, view.animationBindingsMethod.finish()), stmts.push.apply(stmts, view.detectChangesInInputsMethod.finish()), view.viewContainers.forEach(function(viewContainer) {
      ...
      afterContentStmts.length > 0 && stmts.push(new IfStmt(not(DetectChangesVars.throwOnChange), afterContentStmts)), stmts.push.apply(stmts, view.detectChangesRenderPropertiesMethod.finish()), view.viewChildren.forEach(function(viewChild) {
    ...
    function generateVisitRootNodesMethod(view) {
      var cbVar = variable("cb"),
          ctxVar = variable("ctx"),
          stmts = generateVisitNodesStmts(view.rootNodes, cbVar, ctxVar);
    ...
    function generateVisitProjectableNodesMethod(view) {
      var nodeIndexVar = variable("nodeIndex"),
          ngContentIndexVar = variable("ngContentIndex"),
          cbVar = variable("cb"),
          ctxVar = variable("ctx"),
          stmts = [];
      return view.nodes.forEach(function(node) {
        node instanceof CompileElement && node.component && node.contentNodesByNgContentIndex.forEach(function(projectedNodes, ngContentIndex) {
      ...
      }), new ClassMethod("visitProjectableNodesInternal", [new FnParam(nodeIndexVar.name, NUMBER_TYPE), new FnParam(ngContentIndexVar.name, NUMBER_TYPE), new FnParam(cbVar.name, DYNAMIC_TYPE), new FnParam(ctxVar.name, DYNAMIC_TYPE)], stmts);
    }
      ...
      return nodes.forEach(function(node) {
        switch (node.type) {
          ...
          case CompileViewRootNodeType.ViewContainer:
            stmts.push(cb.callFn([node.expr.prop("nativeElement"), ctx]).toStmt()), stmts.push(node.expr.callMethod("visitNestedViewRootNodes", [cb, ctx]).toStmt());
            break;
    ...
    function generateCreateEmbeddedViewsMethod(view) {
      var nodeIndexVar = variable("nodeIndex"),
          stmts = [];
      return view.nodes.forEach(function(node) {
        if (node instanceof CompileElement && node.embeddedView) {
          node.isRootElement() ? null : node.parent.nodeIndex;
      ...
      }), stmts.length > 0 && stmts.push(new ReturnStatement(NULL_EXPR)), new ClassMethod("createEmbeddedViewInternal", [new FnParam(nodeIndexVar.name, NUMBER_TYPE)], stmts, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE]));
    }
    ...
    function _analyzeNgModules(ngModuleMetas) {
      ...
      ngModuleMetas.forEach(function(ngModuleMeta) {
        ...
        filePaths.add(srcFileUrl), ngModulesByFile.set(srcFileUrl, (ngModulesByFile.get(srcFileUrl) || []).concat(ngModuleMeta.type.reference)), ngModuleMeta.declaredDirectives.forEach(function(dirIdentifier) {
          ...
          filePaths.add(fileUrl), ngModuleByPipeOrDirective.set(pipeIdentifier.reference, ngModuleMeta);
        });
      });
      var files = [];
      ...
      var ngModules = new Map,
          ...
          addNgModule = function(staticSymbol) {
            if (ngModules.has(staticSymbol))
              return !1;
            var _a = metadataResolver.loadNgModuleMetadata(staticSymbol, !1, !1),
                ngModule = _a.ngModule,
                loading = _a.loading;
            return ngModule && (ngModules.set(ngModule.type.reference, ngModule), loadingPromises.push(loading), ngModule.declaredDirectives.forEach(function(dir) {
            ...
            }), options.transitiveModules && ngModule.transitiveModule.modules.forEach(function(modMeta) {
    ...
    function _parseAnimationDeclarationStates(stateMetadata, schema, errors) {
      var normalizedStyles = _normalizeStyleMetadata(stateMetadata.styles, {}, schema, errors, !1),
          defStyles = new AnimationStylesAst(normalizedStyles),
    ...
    function _parseAnimationStateTransition(transitionStateMetadata, stateStyles, schema, errors) {
      ...
      var entry = _normalizeAnimationEntry(transitionStateMetadata.steps),
          animation = _normalizeStyleSteps(entry, stateStyles, schema, errors),
    ...
    function _normalizeStyleMetadata(entry, stateStyles, schema, errors, permitStateReferences) {
      ...
      return entry.styles.forEach(function(styleEntry) {
        if ("string" == typeof styleEntry)
          permitStateReferences ? normalizedStyles.push.apply(normalizedStyles, _resolveStylesFromState(styleEntry, stateStyles, errors)) : errors.push(new AnimationParseError("State based animations cannot contain references to other states"));
        else {
          var stylesObj_1 = styleEntry,
              normalizedStylesObj_1 = {};
          Object.keys(stylesObj_1).forEach(function(propName) {
            var normalizedProp = schema.normalizeAnimationStyleProperty(propName),
                normalizedOutput = schema.normalizeAnimationStyleValue(normalizedProp, propName, stylesObj_1[propName]),
                normalizationError = normalizedOutput.error;
            normalizationError && errors.push(new AnimationParseError(normalizationError)), normalizedStylesObj_1[normalizedProp] = normalizedOutput.value;
    ...
    function _normalizeStyleStepEntry(entry, stateStyles, schema, errors) {
      ...
      return steps.forEach(function(step) {
        if (step instanceof CompileAnimationStyleMetadata)
          isPresent(combinedStyles) || (combinedStyles = []), _normalizeStyleMetadata(step, stateStyles, schema, errors, !0).forEach(function(entry) {
          ...
          if (isPresent(combinedStyles) && (newSteps.push(new CompileAnimationStyleMetadata(0, combinedStyles)), combinedStyles = null), step instanceof CompileAnimationAnimateMetadata) {
            ...
            animateStyleValue instanceof CompileAnimationStyleMetadata ? animateStyleValue.styles = _normalizeStyleMetadata(animateStyleValue, stateStyles, schema, errors, !0) : animateStyleValue instanceof CompileAnimationKeyframesSequenceMetadata && animateStyleValue.steps.forEach(function(step) {
              step.styles = _normalizeStyleMetadata(step, stateStyles, schema, errors, !0);
            });
    ...
    function _parseAnimationKeyframes(keyframeSequence, currentTime, collectedStyles, stateStyles, errors) {
      ...
      }), doSortKeyframes && rawKeyframes.sort(function(a, b) {
        return a[0] <= b[0] ? -1 : 1;
      });
      var firstKeyframe = rawKeyframes[0];
      firstKeyframe[0] != _INITIAL_KEYFRAME && rawKeyframes.splice(0, 0, firstKeyframe = [_INITIAL_KEYFRAME, {}]);
    ...
    function _parseTransitionAnimation(entry, currentTime, collectedStyles, stateStyles, errors) {
      ...
      if (entry instanceof CompileAnimationWithStepsMetadata) {
        var previousStyles_1,
            maxDuration_1 = 0,
            steps_1 = [],
            isGroup_1 = entry instanceof CompileAnimationGroupMetadata;
        if (entry.steps.forEach(function(entry) {
          ...
          if (entry instanceof CompileAnimationStyleMetadata)
            ...
            }), void(previousStyles_1 = entry.styles);
          ...
          currentTime += astDuration, playTime += astDuration, maxDuration_1 = Math.max(astDuration, maxDuration_1), steps_1.push(innerAst);
          var _a;
        }), isPresent(previousStyles_1)) {
          var startingStyles = new AnimationStylesAst(previousStyles_1);
          steps_1.push(new AnimationStepAst(startingStyles, [], 0, 0, ""));
        }
        isGroup_1 ? (ast = new AnimationGroupAst(steps_1), playTime = maxDuration_1, currentTime = startingTime + playTime) : ast = new AnimationSequenceAst(steps_1);
    ...
    function getTransitiveExportedModules(modules, targetModules, visitedModules) {
      return void 0 === targetModules && (targetModules = []), void 0 === visitedModules && (visitedModules = new Set), modules.forEach(function(ngModule) {
        visitedModules.has(ngModule.type.reference) || (visitedModules.add(ngModule.type.reference), getTransitiveExportedModules(ngModule.exportedModules, targetModules, visitedModules), targetModules.push(ngModule));
      }), targetModules;
    }
    function getTransitiveImportedModules(modules, targetModules, visitedModules) {
      return void 0 === targetModules && (targetModules = []), void 0 === visitedModules && (visitedModules = new Set), modules.forEach(function(ngModule) {
        if (!visitedModules.has(ngModule.type.reference)) {
          ...
          getTransitiveImportedModules(nestedModules, targetModules, visitedModules), targetModules.push(ngModule);
        }
      }), targetModules;
    }
      ...
      return TextAst.prototype.visit = function(visitor, context) {
        ...
        BoundElementPropertyAst = function() {
          function BoundElementPropertyAst(name, type, securityContext, needsRuntimeSecurityContext, value, unit, sourceSpan) {
            this.name = name, this.type = type, this.securityContext = securityContext, this.needsRuntimeSecurityContext = needsRuntimeSecurityContext, this.value = value, this.unit = unit, this.sourceSpan = sourceSpan;
          }
    ...
    var $EOF = 0,
        ...
        Quote = function(_super) {
          ...
          return __extends$1(Quote, _super), Quote.prototype.visit = function(visitor, context) {
        ...
        EmptyExpr = function(_super) {
          ...
          return __extends$1(EmptyExpr, _super), EmptyExpr.prototype.visit = function(visitor, context) {
            void 0 === context && (context = null);
          }, EmptyExpr;
        }(AST),
        ImplicitReceiver = function(_super) {
          ...
          return __extends$1(ImplicitReceiver, _super), ImplicitReceiver.prototype.visit = function(visitor, context) {
        ...
        Chain = function(_super) {
          ...
          return __extends$1(Chain, _super), Chain.prototype.visit = function(visitor, context) {
        ...
        Conditional = function(_super) {
          ...
          return __extends$1(Conditional, _super), Conditional.prototype.visit = function(visitor, context) {
        ...
        PropertyRead = function(_super) {
          ...
          return __extends$1(PropertyRead, _super), PropertyRead.prototype.visit = function(visitor, context) {
        ...
        PropertyWrite = function(_super) {
          ...
          return __extends$1(PropertyWrite, _super), PropertyWrite.prototype.visit = function(visitor, context) {
        ...
        SafePropertyRead = function(_super) {
          ...
          return __extends$1(SafePropertyRead, _super), SafePropertyRead.prototype.visit = function(visitor, context) {
        ...
        KeyedRead = function(_super) {
          ...
          return __extends$1(KeyedRead, _super), KeyedRead.prototype.visit = function(visitor, context) {
        ...
        KeyedWrite = function(_super) {
          ...
          return __extends$1(KeyedWrite, _super), KeyedWrite.prototype.visit = function(visitor, context) {
        ...
        BindingPipe = function(_super) {
          ...
          return __extends$1(BindingPipe, _super), BindingPipe.prototype.visit = function(visitor, context) {
        ...
        LiteralPrimitive = function(_super) {
          ...
          return __extends$1(LiteralPrimitive, _super), LiteralPrimitive.prototype.visit = function(visitor, context) {
        ...
        LiteralArray = function(_super) {
          ...
          return __extends$1(LiteralArray, _super), LiteralArray.prototype.visit = function(visitor, context) {
        ...
        LiteralMap = function(_super) {
          ...
          return __extends$1(LiteralMap, _super), LiteralMap.prototype.visit = function(visitor, context) {
        ...
        Interpolation = function(_super) {
          ...
          return __extends$1(Interpolation, _super), Interpolation.prototype.visit = function(visitor, context) {
        ...
        Binary = function(_super) {
          ...
          return __extends$1(Binary, _super), Binary.prototype.visit = function(visitor, context) {
        ...
        PrefixNot = function(_super) {
          ...
          return __extends$1(PrefixNot, _super), PrefixNot.prototype.visit = function(visitor, context) {
        ...
        MethodCall = function(_super) {
          ...
          return __extends$1(MethodCall, _super), MethodCall.prototype.visit = function(visitor, context) {
        ...
        SafeMethodCall = function(_super) {
          ...
          return __extends$1(SafeMethodCall, _super), SafeMethodCall.prototype.visit = function(visitor, context) {
        ...
        FunctionCall = function(_super) {
          ...
          return __extends$1(FunctionCall, _super), FunctionCall.prototype.visit = function(visitor, context) {
        ...
        ASTWithSource = function(_super) {
          ...
          return __extends$1(ASTWithSource, _super), ASTWithSource.prototype.visit = function(visitor, context) {
 ...
 while (operatorFound);
            ...
            for (var bindings = [],
              ...
              bindings.push(new TemplateBinding(this.span(start), key, keyIsVar, name_2, expression)), this.optionalCharacter($SEMICOLON) || this.optionalCharacter($COMMA);
            }
    ...
    var _VisitorMode,
        ...
        TokenError = function(_super) {
          ...
          return __extends$3(TokenError, _super), TokenError;
        }(ParseError),
 ...
 while (!this._isTextEnd());
        ...
        TreeError = function(_super) {
          ...
          return __extends$2(TreeError, _super), TreeError.create = function(elementName, span, msg) {
        ...
        I18nError = function(_super) {
          ...
          return __extends$4(I18nError, _super), I18nError;
        }(ParseError),
        _I18N_ATTR = "i18n",
    ...
    var BoundPropertyType,
        ...
        ListWrapper = function() {
          ...
          return ListWrapper.removeAll = function(list, items) {
        ...
        XmlParser = function(_super) {
          ...
          return __extends$5(XmlParser, _super), XmlParser.prototype.parse = function(source, url, parseExpansionForms) {
        ...
        CR = function(_super) {
          ...
          return __extends$6(CR, _super), CR;
        }(Text$2),
        ...
        CompileAnimationStateDeclarationMetadata = function(_super) {
          ...
          return __extends$7(CompileAnimationStateDeclarationMetadata, _super), CompileAnimationStateDeclarationMetadata;
        ...
        CompileAnimationStateTransitionMetadata = function(_super) {
          ...
          return __extends$7(CompileAnimationStateTransitionMetadata, _super), CompileAnimationStateTransitionMetadata;
        }(CompileAnimationStateMetadata),
        ...
        CompileAnimationKeyframesSequenceMetadata = function(_super) {
          ...
          return __extends$7(CompileAnimationKeyframesSequenceMetadata, _super), CompileAnimationKeyframesSequenceMetadata;
        ...
        CompileAnimationStyleMetadata = function(_super) {
          ...
          return __extends$7(CompileAnimationStyleMetadata, _super), CompileAnimationStyleMetadata;
        }(CompileAnimationMetadata),
        CompileAnimationAnimateMetadata = function(_super) {
          ...
          return __extends$7(CompileAnimationAnimateMetadata, _super), CompileAnimationAnimateMetadata;
        }(CompileAnimationMetadata),
        CompileAnimationWithStepsMetadata = function(_super) {
          function CompileAnimationWithStepsMetadata(steps) {
            void 0 === steps && (steps = null), _super.call(this), this.steps = steps;
          }
          return __extends$7(CompileAnimationWithStepsMetadata, _super), CompileAnimationWithStepsMetadata;
        }(CompileAnimationMetadata),
        CompileAnimationSequenceMetadata = function(_super) {
          function CompileAnimationSequenceMetadata(steps) {
            void 0 === steps && (steps = null), _super.call(this, steps);
          }
          return __extends$7(CompileAnimationSequenceMetadata, _super), CompileAnimationSequenceMetadata;
        }(CompileAnimationWithStepsMetadata),
        CompileAnimationGroupMetadata = function(_super) {
          function CompileAnimationGroupMetadata(steps) {
            void 0 === steps && (steps = null), _super.call(this, steps);
          }
          return __extends$7(CompileAnimationGroupMetadata, _super), CompileAnimationGroupMetadata;
        }(CompileAnimationWithStepsMetadata),
        ...
        CompileDiDependencyMetadata = function() {
          function CompileDiDependencyMetadata(_a) {
            var _b = void 0 === _a ? {} : _a,
                ...
                token = _b.token,
                value = _b.value;
            this.isAttribute = !!isAttribute, this.isSelf = !!isSelf, this.isHost = !!isHost, this.isSkipSelf = !!isSkipSelf, this.isOptional = !!isOptional, this.isValue = !!isValue, this.token = token, this.value = value;
          }
        ...
        CompileProviderMetadata = function() {
          function CompileProviderMetadata(_a) {
            var token = _a.token,
                ...
                deps = _a.deps,
                multi = _a.multi;
            this.token = token, this.useClass = useClass, this.useValue = useValue, this.useExisting = useExisting, this.useFactory = useFactory, this.deps = deps || null, this.multi = !!multi;
          }
        ...
        CompileFactoryMetadata = function(_super) {
          ...
          return __extends$7(CompileFactoryMetadata, _super), CompileFactoryMetadata;
        }(CompileIdentifierMetadata),
        CompileTokenMetadata = function() {
          function CompileTokenMetadata(_a) {
            ...
            this.value = value, this.identifier = identifier, this.identifierIsInstance = !!identifierIsInstance;
          }
        ...
        CompileTypeMetadata = function(_super) {
          function CompileTypeMetadata(_a) {
            ...
            _super.call(this, {
              ...
              prefix: prefix,
              value: value
            }), this.isHost = !!isHost, this.diDeps = _normalizeArray(diDeps), this.lifecycleHooks = _normalizeArray(lifecycleHooks);
          }
          return __extends$7(CompileTypeMetadata, _super), CompileTypeMetadata;
        }(CompileIdentifierMetadata),
        CompileQueryMetadata = function() {
          function CompileQueryMetadata(_a) {
            var _b = void 0 === _a ? {} : _a,
                ...
                read = _b.read;
            this.selectors = selectors, this.descendants = !!descendants, this.first = !!first, this.propertyName = propertyName, this.read = read;
          }
        ...
        CompileTemplateMetadata = function() {
          function CompileTemplateMetadata(_a) {
            ...
            if (this.encapsulation = encapsulation, this.template = template, this.templateUrl = templateUrl, this.styles = _normalizeArray(styles), this.styleUrls = _normalizeArray(styleUrls), this.externalStylesheets = _normalizeArray(externalStylesheets), this.animations = animations ? ListWrapper.flatten(animations) : [], this.ngContentSelectors = ngContentSelectors || [], interpolation && 2 != interpolation.length)
        ...
        TransitiveCompileNgModuleMetadata = function() {
          function TransitiveCompileNgModuleMetadata(modules, providers, entryComponents, directives, pipes, loadingPromises) {
            var _this = this;
            this.modules = modules, this.providers = providers, this.entryComponents = entryComponents, this.directives = directives, this.pipes = pipes, this.loadingPromises = loadingPromises, this.directivesSet = new Set, this.pipesSet = new Set, directives.forEach(function(dir) {
        ...
        ViewContainer = _angular_core.__core_private__.ViewContainer,
        CodegenComponentFactoryResolver = _angular_core.__core_private__.CodegenComponentFactoryResolver,
        ComponentRef_ = _angular_core.__core_private__.ComponentRef_,
        ...
        HtmlParser = function(_super) {
          ...
          return __extends$8(HtmlParser, _super), HtmlParser.prototype.parse = function(source, url, parseExpansionForms, interpolationConfig) {
        ...
        ExpansionError = function(_super) {
          ...
          return __extends$9(ExpansionError, _super), ExpansionError;
        }(ParseError),
        _Expander = function() {
          function _Expander() {
        ...
        ProviderError = function(_super) {
          ...
          return __extends$10(ProviderError, _super), ProviderError;
        }(ParseError),
        ...
        ProviderElementContext = function() {
          function ProviderElementContext(viewContext, _parent, _isViewRoot, _directiveAsts, attrs, refs, _sourceSpan) {
            ...
            }), isPresent(queriedTokens.get(resolveIdentifierToken(Identifiers.ViewContainerRef).reference)) && (this._hasViewContainer = !0), Array.from(this._allProviders.values()).forEach(function(provider) {
          ...
          return ProviderElementContext.prototype.afterElement = function() {
            ...
            for (var queries,
              ...
              queries = currentEl._contentQueries.get(token.reference), isPresent(queries) && result.push.apply(result, queries.filter(function(query) {
                return query.descendants || distance <= 1;
              })), currentEl._directiveAsts.length > 0 && distance++, currentEl = currentEl._parent;
            return queries = this.viewContext.viewQueries.get(token.reference), isPresent(queries) && result.push.apply(result, queries), result;
    ...
    !function(BoundPropertyType) {
      BoundPropertyType[BoundPropertyType.DEFAULT = 0] = "DEFAULT", BoundPropertyType[BoundPropertyType.LITERAL_ATTR = 1] = "LITERAL_ATTR", BoundPropertyType[BoundPropertyType.ANIMATION = 2] = "ANIMATION";
    ...
    var PreparsedElementType,
        ...
        BindingParser = function() {
          function BindingParser(_exprParser, _interpolationConfig, _schemaRegistry, pipes, _targetErrors) {
            var _this = this;
            this._exprParser = _exprParser, this._interpolationConfig = _interpolationConfig, this._schemaRegistry = _schemaRegistry, this._targetErrors = _targetErrors, this.pipesByName = new Map, pipes.forEach(function(pipe) {
              return _this.pipesByName.set(pipe.name, pipe);
            });
          }
          return BindingParser.prototype.createDirectiveHostPropertyAsts = function(dirMeta, sourceSpan) {
            var _this = this;
            ...
            if (boundProp.isAnimation)
              return new BoundElementPropertyAst(boundProp.name, exports.PropertyBindingType.Animation, _angular_core.SecurityContext.NONE, (!1), boundProp.expression, null, boundProp.sourceSpan);
            var bindingType,
                boundPropertyName,
                securityContexts,
                unit = null,
            ...
            if (1 === parts.length) {
              ...
              boundPropertyName = this._schemaRegistry.getMappedPropName(partValue), securityContexts = calcPossibleSecurityContexts(this._schemaRegistry, elementSelector, boundPropertyName, !1), bindingType = exports.PropertyBindingType.Property, this._validatePropertyOrAttributeName(boundPropertyName, boundProp.sourceSpan, !1);
              ...
              parts[0] == CLASS_PREFIX ? (boundPropertyName = parts[1], bindingType = exports.PropertyBindingType.Class, securityContexts = [_angular_core.SecurityContext.NONE]) : parts[0] == STYLE_PREFIX ? (unit = parts.length > 2 ? parts[2] : null, boundPropertyName = parts[1], bindingType = exports.PropertyBindingType.Style, securityContexts = [_angular_core.SecurityContext.STYLE]) : (this._reportError("Invalid property name '" + boundProp.name + "'", boundProp.sourceSpan), bindingType = null, securityContexts = []);
            return new BoundElementPropertyAst(boundPropertyName, bindingType, 1 === securityContexts.length ? securityContexts[0] : null, securityContexts.length > 1, boundProp.expression, unit, boundProp.sourceSpan);
            ...
            void 0 === level && (level = exports.ParseErrorLevel.FATAL), this._targetErrors.push(new ParseError(sourceSpan, message, level));
        ...
        PipeCollector = function(_super) {
          ...
          return __extends$11(PipeCollector, _super), PipeCollector.prototype.visitPipe = function(ast, context) {
          ...
          return TemplateParser.prototype.parse = function(component, template, directives, pipes, schemas, templateUrl) {
            ...
            if (htmlAstWithErrors.rootNodes.length > 0) {
              ...
              var bindingParser = new BindingParser(this._exprParser, interpolationConfig, this._schemaRegistry, uniqPipes, errors),
                  parseVisitor = new TemplateParseVisitor(providerViewContext, uniqDirectives, bindingParser, this._schemaRegistry, schemas, errors);
              result = visitAll(parseVisitor, htmlAstWithErrors.rootNodes, EMPTY_ELEMENT_CONTEXT), errors.push.apply(errors, providerViewContext.errors);
            } else
              result = [];
          ...
          return TemplateParseVisitor.prototype.visitExpansion = function(expansion, context) {
            ...
            element.attrs.forEach(function(attr) {
              var hasBinding = _this._parseAttr(isTemplateElement, attr, matchableAttrs, elementOrDirectiveProps, events, elementOrDirectiveRefs, elementVars),
                  templateBindingsSource = void 0,
                  prefixToken = void 0;
              _this._normalizeAttributeName(attr.name) == TEMPLATE_ATTR ? templateBindingsSource = attr.value : attr.name.startsWith(TEMPLATE_ATTR_PREFIX) && (templateBindingsSource = attr.value, prefixToken = attr.name.substring(TEMPLATE_ATTR_PREFIX.length));
            ...
            var elementCssSelector = createElementCssSelector(nodeName, matchableAttrs),
                ...
                elementProps = this._createElementPropertyAsts(element.name, elementOrDirectiveProps, directiveAsts),
            ...
            var _this = this,
                ...
                directiveAsts = directives.map(function(directive) {
                  ...
                  var directiveProperties = [],
                      hostProperties = _this._bindingParser.createDirectiveHostPropertyAsts(directive, sourceSpan);
                  ...
                  return _this._createDirectivePropertyAsts(directive.inputs, props, directiveProperties), elementOrDirectiveRefs.forEach(function(elOrDirRef) {
            ...
            return directives.forEach(function(directive) {
              ...
              prop.isLiteral || boundDirectivePropsIndex.get(prop.name) || boundElementProps.push(_this._bindingParser.createElementPropertyAst(elementName, prop));
            }), this._checkPropertiesInSchema(elementName, boundElementProps), boundElementProps;
    ...
    !function(BuiltinTypeName) {
      BuiltinTypeName[BuiltinTypeName.Dynamic = 0] = "Dynamic", BuiltinTypeName[BuiltinTypeName.Bool = 1] = "Bool", BuiltinTypeName[BuiltinTypeName.String = 2] = "String", BuiltinTypeName[BuiltinTypeName.Int = 3] = "Int", BuiltinTypeName[BuiltinTypeName.Number = 4] = "Number", BuiltinTypeName[BuiltinTypeName.Function = 5] = "Function", BuiltinTypeName[BuiltinTypeName.Null = 6] = "Null";
    ...
    var BinaryOperator,
        BuiltinType = function(_super) {
          ...
          return __extends$12(BuiltinType, _super), BuiltinType.prototype.visitType = function(visitor, context) {
        ...
        ExternalType = function(_super) {
          ...
          return __extends$12(ExternalType, _super), ExternalType.prototype.visitType = function(visitor, context) {
        ...
        ArrayType = function(_super) {
          ...
          return __extends$12(ArrayType, _super), ArrayType.prototype.visitType = function(visitor, context) {
        ...
        MapType = function(_super) {
          ...
          return __extends$12(MapType, _super), MapType.prototype.visitType = function(visitor, context) {
        ...
        NULL_TYPE = new BuiltinType(BuiltinTypeName.Null);
    ...
    var BuiltinMethod,
        ReadVarExpr = function(_super) {
          ...
          return __extends$12(ReadVarExpr, _super), ReadVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WriteVarExpr = function(_super) {
          ...
          return __extends$12(WriteVarExpr, _super), WriteVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WriteKeyExpr = function(_super) {
          ...
          return __extends$12(WriteKeyExpr, _super), WriteKeyExpr.prototype.visitExpression = function(visitor, context) {
        ...
        WritePropExpr = function(_super) {
          ...
          return __extends$12(WritePropExpr, _super), WritePropExpr.prototype.visitExpression = function(visitor, context) {
    ...
    var StmtModifier,
        InvokeMethodExpr = function(_super) {
          ...
          return __extends$12(InvokeMethodExpr, _super), InvokeMethodExpr.prototype.visitExpression = function(visitor, context) {
        ...
        InvokeFunctionExpr = function(_super) {
          ...
          return __extends$12(InvokeFunctionExpr, _super), InvokeFunctionExpr.prototype.visitExpression = function(visitor, context) {
        ...
        InstantiateExpr = function(_super) {
          ...
          return __extends$12(InstantiateExpr, _super), InstantiateExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralExpr = function(_super) {
          ...
          return __extends$12(LiteralExpr, _super), LiteralExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ExternalExpr = function(_super) {
          ...
          return __extends$12(ExternalExpr, _super), ExternalExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ConditionalExpr = function(_super) {
          ...
          return __extends$12(ConditionalExpr, _super), ConditionalExpr.prototype.visitExpression = function(visitor, context) {
        ...
        NotExpr = function(_super) {
          ...
          return __extends$12(NotExpr, _super), NotExpr.prototype.visitExpression = function(visitor, context) {
        ...
        CastExpr = function(_super) {
          ...
          return __extends$12(CastExpr, _super), CastExpr.prototype.visitExpression = function(visitor, context) {
        ...
        FunctionExpr = function(_super) {
          ...
          return __extends$12(FunctionExpr, _super), FunctionExpr.prototype.visitExpression = function(visitor, context) {
        ...
        BinaryOperatorExpr = function(_super) {
          ...
          return __extends$12(BinaryOperatorExpr, _super), BinaryOperatorExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ReadPropExpr = function(_super) {
          ...
          return __extends$12(ReadPropExpr, _super), ReadPropExpr.prototype.visitExpression = function(visitor, context) {
        ...
        ReadKeyExpr = function(_super) {
          ...
          return __extends$12(ReadKeyExpr, _super), ReadKeyExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralArrayExpr = function(_super) {
          ...
          return __extends$12(LiteralArrayExpr, _super), LiteralArrayExpr.prototype.visitExpression = function(visitor, context) {
        ...
        LiteralMapExpr = function(_super) {
          ...
          return __extends$12(LiteralMapExpr, _super), LiteralMapExpr.prototype.visitExpression = function(visitor, context) {
        ...
        TYPED_NULL_EXPR = new LiteralExpr(null, NULL_TYPE);
    ...
    var _Mode,
        ...
        DeclareVarStmt = function(_super) {
          ...
          return __extends$12(DeclareVarStmt, _super), DeclareVarStmt.prototype.visitStatement = function(visitor, context) {
        ...
        DeclareFunctionStmt = function(_super) {
          ...
          return __extends$12(DeclareFunctionStmt, _super), DeclareFunctionStmt.prototype.visitStatement = function(visitor, context) {
        ...
        ExpressionStatement = function(_super) {
          ...
          return __extends$12(ExpressionStatement, _super), ExpressionStatement.prototype.visitStatement = function(visitor, context) {
        ...
        ReturnStatement = function(_super) {
          ...
          return __extends$12(ReturnStatement, _super), ReturnStatement.prototype.visitStatement = function(visitor, context) {
        ...
        ClassField = function(_super) {
          ...
          return __extends$12(ClassField, _super), ClassField;
        }(AbstractClassPart),
        ClassMethod = function(_super) {
          ...
          return __extends$12(ClassMethod, _super), ClassMethod;
        }(AbstractClassPart),
        ClassGetter = function(_super) {
          ...
          return __extends$12(ClassGetter, _super), ClassGetter;
        }(AbstractClassPart),
        ClassStmt = function(_super) {
          ...
          return __extends$12(ClassStmt, _super), ClassStmt.prototype.visitStatement = function(visitor, context) {
        ...
        IfStmt = function(_super) {
          ...
          return __extends$12(IfStmt, _super), IfStmt.prototype.visitStatement = function(visitor, context) {
        ...
        TryCatchStmt = (function(_super) {
          ...
          return __extends$12(CommentStmt, _super), CommentStmt.prototype.visitStatement = function(visitor, context) {
          ...
          return __extends$12(TryCatchStmt, _super), TryCatchStmt.prototype.visitStatement = function(visitor, context) {
        ...
        ThrowStmt = function(_super) {
          ...
          return __extends$12(ThrowStmt, _super), ThrowStmt.prototype.visitStatement = function(visitor, context) {
        ...
        _ReplaceVariableTransformer = function(_super) {
          ...
          return __extends$12(_ReplaceVariableTransformer, _super), _ReplaceVariableTransformer.prototype.visitReadVarExpr = function(ast, context) {
        ...
        _VariableFinder = function(_super) {
          ...
          return __extends$12(_VariableFinder, _super), _VariableFinder.prototype.visitReadVarExpr = function(ast, context) {
        ...
        AnimationStateAst = function(_super) {
          ...
          return __extends$13(AnimationStateAst, _super), AnimationStateAst;
        }(AnimationAst),
        AnimationEntryAst = function(_super) {
          ...
          return __extends$13(AnimationEntryAst, _super), AnimationEntryAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateDeclarationAst = function(_super) {
          ...
          return __extends$13(AnimationStateDeclarationAst, _super), AnimationStateDeclarationAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStateTransitionAst = function(_super) {
          ...
          return __extends$13(AnimationStateTransitionAst, _super), AnimationStateTransitionAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStepAst = function(_super) {
          ...
          return __extends$13(AnimationStepAst, _super), AnimationStepAst.prototype.visit = function(visitor, context) {
        ...
        AnimationStylesAst = function(_super) {
          ...
          return __extends$13(AnimationStylesAst, _super), AnimationStylesAst.prototype.visit = function(visitor, context) {
        ...
        AnimationKeyframeAst = function(_super) {
          ...
          return __extends$13(AnimationKeyframeAst, _super), AnimationKeyframeAst.prototype.visit = function(visitor, context) {
        ...
        AnimationWithStepsAst = function(_super) {
          ...
          return __extends$13(AnimationWithStepsAst, _super), AnimationWithStepsAst;
        }(AnimationAst),
        AnimationGroupAst = function(_super) {
          function AnimationGroupAst(steps) {
            _super.call(this, steps);
          }
          return __extends$13(AnimationGroupAst, _super), AnimationGroupAst.prototype.visit = function(visitor, context) {
        ...
        AnimationSequenceAst = function(_super) {
          ...
          return __extends$13(AnimationSequenceAst, _super), AnimationSequenceAst.prototype.visit = function(visitor, context) {
        ...
        CheckBindingField = function() {
          function CheckBindingField(expression, bindingId) {
            this.expression = expression, this.bindingId = bindingId;
          }
          return CheckBindingField;
        }(),
        VAL_UNWRAPPER_VAR = variable("valUnwrapper"),
        ...
        ConvertPropertyBindingResult = function() {
          function ConvertPropertyBindingResult(stmts, currValExpr, forceUpdate) {
            this.stmts = stmts, this.currValExpr = currValExpr, this.forceUpdate = forceUpdate;
          }
          return ConvertPropertyBindingResult;
        }(),
        ConvertActionBindingResult = function() {
          function ConvertActionBindingResult(stmts, preventDefault) {
            this.stmts = stmts, this.preventDefault = preventDefault;
          }
    ...
    var CompileViewRootNodeType,
        _AstToIrVisitor = function() {
          function _AstToIrVisitor(_builder, _nameResolver, _implicitReceiver, _valueUnwrapper, bindingId, isAction) {
            this._builder = _builder, this._nameResolver = _nameResolver, this._implicitReceiver = _implicitReceiver, this._valueUnwrapper = _valueUnwrapper, this.bindingId = bindingId, this.isAction = isAction, this._nodeMap = new Map, this._resultMap = new Map, this._currentTemporary = 0, this.needsValueUnwrapper = !1, this.temporaryCount = 0;
          }
          return _AstToIrVisitor.prototype.visitBinary = function(ast, mode) {
            ...
            return args.push(literal(ast.strings[ast.strings.length - 1])), ast.expressions.length <= 9 ? importExpr(resolveIdentifier(Identifiers.inlineInterpolate)).callFn(args) : importExpr(resolveIdentifier(Identifiers.interpolate)).callFn([args[0], literalArr(args.slice(1))]);
            ...
            var parts = this.visitAll(ast.expressions, mode),
                literalArr$$ = this.isAction ? literalArr(parts) : createCachedLiteralArray(this._builder, parts);
            ...
            return this.isAction && name == EventHandlerVars.event.name ? EventHandlerVars.event : this._nameResolver.getLocal(name);
            ...
            var result = null,
                receiver = this.visit(ast.receiver, _Mode.Expression);
            return receiver === this._implicitReceiver && (result = this._getLocal(ast.name)), isBlank(result) && (result = receiver.prop(ast.name)), convertToStatementIfNeeded(mode, result);
        ...
        DefaultNameResolver = function() {
          ...
          return DefaultNameResolver.prototype.callPipe = function(name, input, args) {
            return null;
        ...
        CHANGES_FIELD_NAME = "_changes",
        CHANGED_FIELD_NAME = "_changed",
        EVENT_HANDLER_FIELD_NAME = "_eventHandler",
        ...
        COMPONENT_VIEW_VAR = variable("componentView"),
        RENDER_EL_VAR = variable("el"),
        EVENT_NAME_VAR = variable("eventName"),
        ...
        DirectiveWrapperCompiler = function() {
          function DirectiveWrapperCompiler(compilerConfig, _exprParser, _schemaRegistry, _console) {
            this.compilerConfig = compilerConfig, this._exprParser = _exprParser, this._schemaRegistry = _schemaRegistry, this._console = _console;
          }
          return DirectiveWrapperCompiler.dirWrapperClassName = function(id) {
            ...
            }), addNgDoCheckMethod(builder), addCheckHostMethod(hostParseResult.hostProps, builder), addHandleEventMethod(hostParseResult.hostListeners, builder), addSubscribeMethod(dirMeta, builder);
            var classStmt = builder.build();
            return new DirectiveWrapperCompileResult([classStmt], classStmt.name);
          }, DirectiveWrapperCompiler.decorators = [{type: _angular_core.Injectable}], DirectiveWrapperCompiler.ctorParameters = [{type: CompilerConfig}, {type: Parser}, {type: ElementSchemaRegistry}, {type: Console}], DirectiveWrapperCompiler;
        }(),
        DirectiveWrapperBuilder = function() {
          function DirectiveWrapperBuilder(compilerConfig, dirMeta) {
            this.compilerConfig = compilerConfig, this.dirMeta = dirMeta, this.fields = [], this.getters = [], this.methods = [], this.ctorStmts = [], this.detachStmts = [], this.destroyStmts = [];
          ...
          return DirectiveWrapperBuilder.prototype.build = function() {
            for (var dirDepParamNames = [],
                i = 0; i < this.dirMeta.type.diDeps.length; i++)
            ...
            var methods = [new ClassMethod("ngOnDetach", [new FnParam(VIEW_VAR.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(COMPONENT_VIEW_VAR.name, importType(resolveIdentifier(Identifiers.AppView), [DYNAMIC_TYPE])), new FnParam(RENDER_EL_VAR.name, DYNAMIC_TYPE)], this.detachStmts), new ClassMethod("ngOnDestroy", [], this.destroyStmts)],
                fields = [new ClassField(EVENT_HANDLER_FIELD_NAME, FUNCTION_TYPE, [StmtModifier.Private]), new ClassField(CONTEXT_FIELD_NAME, importType(this.dirMeta.type)), new ClassField(CHANGED_FIELD_NAME, BOOL_TYPE, [StmtModifier.Private])],
                ctorStmts = [THIS_EXPR.prop(CHANGED_FIELD_NAME).set(literal(!1)).toStmt()];
            return this.genChanges && (fields.push(new ClassField(CHANGES_FIELD_NAME, new MapType(DYNAMIC_TYPE), [StmtModifier.Private])), ctorStmts.push(RESET_CHANGES_STMT)), ctorStmts.push(THIS_EXPR.prop(CONTEXT_FIELD_NAME).set(importExpr(this.dirMeta.type).instantiate(dirDepParamNames.map(function(paramName) {
              return variable(paramName);
            }))).toStmt()), createClassStmt({
        ...
        ParseResult = function() {
          function ParseResult(hostProps, hostListeners, errors) {
            this.hostProps = hostProps, this.hostListeners = hostListeners, this.errors = errors;
          }
          return ParseResult;
        }(),
        DirectiveWrapperExpressions = function() {
          function DirectiveWrapperExpressions() {}
          return DirectiveWrapperExpressions.create = function(dir, depsExpr) {
            ...
            return hostProps.length ? [dirWrapper.callMethod("checkHost", [view, componentView, renderElement, throwOnChange].concat(runtimeSecurityContexts)).toStmt()] : [];
          }, DirectiveWrapperExpressions.ngOnDetach = function(hostProps, dirWrapper, view, componentView, renderEl) {
            return hostProps.some(function(prop) {
              return prop.isAnimation;
            }) ? [dirWrapper.callMethod("ngOnDetach", [view, componentView, renderEl]).toStmt()] : [];
          }, DirectiveWrapperExpressions.ngOnDestroy = function(dir, dirWrapper) {
            return dir.type.lifecycleHooks.indexOf(LifecycleHooks.OnDestroy) !== -1 || Object.keys(dir.outputs).length > 0 ? [dirWrapper.callMethod("ngOnDestroy", []).toStmt()] : [];
          }, DirectiveWrapperExpressions.subscribe = function(dirMeta, hostProps, usedEvents, dirWrapper, view, eventListener) {
            var needsSubscribe = !1,
                eventFlags = [];
            return Object.keys(dirMeta.outputs).forEach(function(propName) {
              var eventName = dirMeta.outputs[propName],
                  eventUsed = usedEvents.indexOf(eventName) > -1;
              needsSubscribe = needsSubscribe || eventUsed, eventFlags.push(literal(eventUsed));
            }), hostProps.forEach(function(hostProp) {
              hostProp.isAnimation && usedEvents.length > 0 && (needsSubscribe = !0);
            }), needsSubscribe ? [dirWrapper.callMethod("subscribe", [view, eventListener].concat(eventFlags)).toStmt()] : [];
        ...
        CompileMethod = function() {
          ...
          return CompileMethod.prototype._updateDebugContextIfNeeded = function() {
            ...
            this._updateDebugContextIfNeeded(), (_a = this._bodyStatements).push.apply(_a, stmts);
            var _a;
        ...
        ViewConstructorVars = function() {
          ...
          return ViewConstructorVars.viewUtils = variable("viewUtils"), ViewConstructorVars.parentView = variable("parentView"), ViewConstructorVars.parentIndex = variable("parentIndex"), ViewConstructorVars.parentElement = variable("parentElement"), ViewConstructorVars;
        }(),
        ViewProperties = function() {
          function ViewProperties() {}
          return ViewProperties.renderer = THIS_EXPR.prop("renderer"), ViewProperties.viewUtils = THIS_EXPR.prop("viewUtils"), ViewProperties;
        }(),
        ...
        DetectChangesVars = function() {
          ...
          return DetectChangesVars.throwOnChange = variable("throwOnChange"), DetectChangesVars.changes = variable("changes"), DetectChangesVars.changed = variable("changed"), DetectChangesVars;
        }(),
        ViewClassDependency = function() {
        ...
        CompileElement = function(_super) {
          ...
          return __extends$14(CompileElement, _super), CompileElement.createNull = function() {
            ...
            this.view.createMethod.addStmt(statement), this.viewContainer = THIS_EXPR.prop(fieldName), this.instances.set(resolveIdentifierToken(Identifiers.ViewContainer).reference, this.viewContainer), this.view.viewContainers.push(this.viewContainer);
            ...
            if (entryComponents && 0 !== entryComponents.length) {
              ...
              })), injectFromViewParentInjector(this.view, resolveIdentifierToken(Identifiers.ComponentFactoryResolver), !1)]),
            ...
            this.compViewExpr = compViewExpr, this.contentNodesByNgContentIndex = new Array(this.component.template.ngContentSelectors.length);
            ...
            var _this = this;
            this.hasViewContainer && this.instances.set(resolveIdentifierToken(Identifiers.ViewContainerRef).reference, this.viewContainer.prop("vcRef")), this._resolvedProviders = new Map, this._resolvedProvidersArray.forEach(function(provider) {
                  ...
                  providerValueExpressions = resolvedProvider.providers.map(function(provider) {
                    ...
                    if (provider.useClass) {
                      ...
                      if (isDirectiveWrapper) {
                        ...
                        return _this._targetDependencies.push(new DirectiveWrapperDependency(provider.useClass, directiveWrapperIdentifier)), DirectiveWrapperExpressions.create(directiveWrapperIdentifier, depsExpr);
                      }
              ...
              isDirectiveWrapper ? (_this.directiveWrapperInstance.set(resolvedProvider.token.reference, instance), _this.instances.set(resolvedProvider.token.reference, DirectiveWrapperExpressions.context(instance))) : _this.instances.set(resolvedProvider.token.reference, instance);
            });
            for (var _loop_1 = function(i) {
              var directive = this_1._directives[i],
                  directiveInstance = this_1.instances.get(identifierToken(directive.type).reference);
              directive.queries.forEach(function(queryMeta) {
                _this._addQuery(queryMeta, directiveInstance);
              });
            },
                this_1 = this,
                i = 0; i < this._directives.length; i++)
              _loop_1(i);
            ...
            Array.from(this._resolvedProviders.values()).forEach(function(resolvedProvider) {
              ...
              varValue = token ? _this.instances.get(token.reference) : _this.renderNode, _this.view.locals.set(varName, varValue);
            ...
            for (var queries,
              ...
              queries = currentEl._queries.get(token.reference), isPresent(queries) && result.push.apply(result, queries.filter(function(query) {
                return query.meta.descendants || distance <= 1;
              })), currentEl._directives.length > 0 && distance++, currentEl = currentEl.parent;
            return queries = this.view.componentView.viewQueries.get(token.reference), isPresent(queries) && result.push.apply(result, queries), result;
            ...
            if (isPresent(dep.token)) {
              if (!result && dep.token.reference === resolveIdentifierToken(Identifiers.ChangeDetectorRef).reference)
                return requestingProviderType === exports.ProviderAstType.Component ? this.compViewExpr.prop("ref") : getPropertyInView(THIS_EXPR.prop("ref"), this.view, this.view.componentView);
              if (!result) {
            ...
            return result || (result = injectFromViewParentInjector(this.view, dep.token, dep.isOptional)), result || (result = NULL_EXPR), getPropertyInView(result, this.view, currElement.view);
          }, CompileElement;
        }(CompileNode),
        ...
        CompilePipe = function() {
          function CompilePipe(view, meta) {
            ...
            var deps = this.meta.type.diDeps.map(function(diDep) {
              return diDep.token.reference === resolveIdentifierToken(Identifiers.ChangeDetectorRef).reference ? getPropertyInView(THIS_EXPR.prop("ref"), _this.view, _this.view.componentView) : injectFromViewParentInjector(view, diDep.token, !1);
            });
          ...
          return CompilePipe.call = function(view, name, args) {
            ...
            if (this.meta.pure) {
              ...
              }), importExpr(resolveIdentifier(Identifiers.castByValue)).callFn([purePipeProxyInstance, pipeInstanceSeenFromPureProxy.prop("transform")]).callFn(args);
            }
            return getPropertyInView(this.instance, callingView, this.view).callMethod("transform", args);
          }, CompilePipe;
        }();
    !function(CompileViewRootNodeType) {
      CompileViewRootNodeType[CompileViewRootNodeType.Node = 0] = "Node", CompileViewRootNodeType[CompileViewRootNodeType.ViewContainer = 1] = "ViewContainer", CompileViewRootNodeType[CompileViewRootNodeType.NgContent = 2] = "NgContent";
    }(CompileViewRootNodeType || (CompileViewRootNodeType = {}));
    var _ComponentIndex,
        CompileViewRootNode = function() {
          function CompileViewRootNode(type, expr, ngContentIndex) {
            this.type = type, this.expr = expr, this.ngContentIndex = ngContentIndex;
          }
            ...
            if (this.viewType === ViewType.COMPONENT) {
              ...
              this.component.viewQueries.forEach(function(queryMeta, queryIndex) {
                var propName = "_viewQuery_" + queryMeta.selectors[0].name + "_" + queryIndex,
                    queryList = createQueryList(queryMeta, directiveInstance_1, propName, _this),
                    query = new CompileQuery(queryMeta, queryList, directiveInstance_1, _this);
        ...
        ViewBinderVisitor = (not(DetectChangesVars.throwOnChange), function() {
          function ViewBinderVisitor(view, _schemaRegistry) {
            this.view = view, this._schemaRegistry = _schemaRegistry, this._nodeIndex = 0;
          }
          return ViewBinderVisitor.prototype.visitBoundText = function(ast, parent) {
            ...
            var _this = this,
                compileElement = this.view.nodes[this._nodeIndex++],
                hasEvents = bindOutputs(ast.outputs, ast.directives, compileElement, !0);
            return bindRenderInputs(ast.inputs, hasEvents, compileElement), ast.directives.forEach(function(directiveAst, dirIndex) {
              var directiveWrapperInstance = compileElement.directiveWrapperInstance.get(directiveAst.directive.type.reference);
              bindDirectiveInputs(directiveAst, directiveWrapperInstance, dirIndex, compileElement), bindDirectiveHostProps(directiveAst, directiveWrapperInstance, compileElement, ast.name, _this._schemaRegistry);
              ...
              bindDirectiveAfterContentLifecycleCallbacks(directiveAst.directive, directiveInstance, compileElement), bindDirectiveAfterViewLifecycleCallbacks(directiveAst.directive, directiveInstance, compileElement), bindDirectiveWrapperLifecycleCallbacks(directiveAst, directiveWrapperInstance, compileElement);
            ...
            return bindOutputs(ast.outputs, ast.directives, compileElement, !1), ast.directives.forEach(function(directiveAst, dirIndex) {
              var directiveInstance = compileElement.instances.get(directiveAst.directive.type.reference),
                  directiveWrapperInstance = compileElement.directiveWrapperInstance.get(directiveAst.directive.type.reference);
              bindDirectiveInputs(directiveAst, directiveWrapperInstance, dirIndex, compileElement), bindDirectiveAfterContentLifecycleCallbacks(directiveAst.directive, directiveInstance, compileElement), bindDirectiveAfterViewLifecycleCallbacks(directiveAst.directive, directiveInstance, compileElement), bindDirectiveWrapperLifecycleCallbacks(directiveAst, directiveWrapperInstance, compileElement);
            }), ast.providers.forEach(function(providerAst) {
              var providerInstance = compileElement.instances.get(providerAst.token.reference);
              bindInjectableDestroyLifecycleCallbacks(providerAst, providerInstance, compileElement);
            }), bindView(compileElement.embeddedView, ast.children, this._schemaRegistry), null;
        ...
        ViewBuilderVisitor = function() {
          ...
          return ViewBuilderVisitor.prototype._isRootNode = function(parent) {
            ...
            var renderNode = THIS_EXPR.prop(fieldName),
                compileElement = new CompileElement(parent, this.view, nodeIndex, renderNode, ast, component, directives, ast.providers, ast.hasViewContainer, (!1), ast.references, this.targetDependencies);
            this.view.nodes.push(compileElement);
            var compViewExpr = null;
            if (isPresent(component)) {
              var nestedComponentIdentifier = new CompileIdentifierMetadata({name: getViewClassName(component, 0)});
              this.targetDependencies.push(new ViewClassDependency(component.type, nestedComponentIdentifier)), compViewExpr = THIS_EXPR.prop("compView_" + nodeIndex), this.view.fields.push(new ClassField(compViewExpr.name, importType(resolveIdentifier(Identifiers.AppView), [importType(component.type)]))), this.view.viewChildren.push(compViewExpr), compileElement.setComponentView(compViewExpr), this.view.createMethod.addStmt(compViewExpr.set(importExpr(nestedComponentIdentifier).instantiate([ViewProperties.viewUtils, THIS_EXPR, literal(nodeIndex), renderNode])).toStmt());
            }
            return compileElement.beforeChildren(), this._addRootNodeAndProject(compileElement), templateVisitAll(this, ast.children, compileElement), compileElement.afterChildren(this.view.nodes.length - nodeIndex - 1), isPresent(compViewExpr) && this.view.createMethod.addStmt(compViewExpr.callMethod("create", [compileElement.getComponent()]).toStmt()), null;
        ...
        ViewCompileResult = function() {
          function ViewCompileResult(statements, viewClassVar, dependencies) {
            this.statements = statements, this.viewClassVar = viewClassVar, this.dependencies = dependencies;
          }
          return ViewCompileResult;
        }(),
        ViewCompiler = function() {
          function ViewCompiler(_genConfig, _schemaRegistry) {
            this._genConfig = _genConfig, this._schemaRegistry = _schemaRegistry;
          }
          return ViewCompiler.prototype.compileComponent = function(component, template, styles, pipes, compiledAnimations) {
            var dependencies = [],
                ...
                statements = [];
            return buildView(view, template, dependencies), bindView(view, template, this._schemaRegistry), finishView(view, statements), new ViewCompileResult(statements, view.classExpr.name, dependencies);
          }, ViewCompiler.decorators = [{type: _angular_core.Injectable}], ViewCompiler.ctorParameters = [{type: CompilerConfig}, {type: ElementSchemaRegistry}], ViewCompiler;
        }(),
        SourceModule = function() {
          function SourceModule(fileUrl, moduleUrl, source) {
            this.fileUrl = fileUrl, this.moduleUrl = moduleUrl, this.source = source;
          }
          return SourceModule;
        }(),
        OfflineCompiler = function() {
          ...
          return OfflineCompiler.prototype.clearCache = function() {
            ...
            var _this = this,
                fileSuffix = _splitTypescriptSuffix(srcFileUrl)[1],
                statements = [],
                exportedVars = [],
                outputSourceModules = [];
            if (exportedVars.push.apply(exportedVars, ngModules.map(function(ngModuleType) {
            ...
            var hostMeta = createHostComponentMeta(compMeta),
                hostViewFactoryVar = this._compileComponent(hostMeta, ngModule, [compMeta.type], null, fileSuffix, targetStatements),
            ...
            var _this = this,
                ...
                parsedTemplate = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, ngModule.schemas, compMeta.type.name),
            ...
            }), targetStatements.push.apply(targetStatements, _resolveViewStatements(viewResult)), viewResult.viewClassVar;
          }, OfflineCompiler.prototype._codgenStyles = function(fileUrl, stylesCompileResult, fileSuffix) {
            return _resolveStyleStatements(stylesCompileResult, fileSuffix), this._codegenSourceModule(fileUrl, _stylesModuleUrl(stylesCompileResult.meta.moduleUrl, stylesCompileResult.isShimmed, fileSuffix), stylesCompileResult.statements, [stylesCompileResult.stylesVar]);
        ...
        AnimationParseError = function(_super) {
          ...
          return __extends$16(AnimationParseError, _super), AnimationParseError.prototype.toString = function() {
            return "" + this.msg;
          }, AnimationParseError;
        }(ParseError),
        ...
        AnimationParser = function() {
          ...
          return AnimationParser.prototype.parseComponent = function(component) {
            ...
            entry.definitions.forEach(function(def) {
              def instanceof CompileAnimationStateDeclarationMetadata ? _parseAnimationDeclarationStates(def, _this._schema, errors).forEach(function(ast) {
          ...
          }, AnimationParser.decorators = [{type: _angular_core.Injectable}], AnimationParser.ctorParameters = [{type: ElementSchemaRegistry}], AnimationParser;
        }(),
    ...
    var DirectiveNormalizer = function() {
      ...
      return DirectiveNormalizer.prototype.clearCache = function() {
        ...
        var interpolationConfig = InterpolationConfig.fromArray(prenomData.interpolation),
            rootNodesAndErrors = this._htmlParser.parse(template, stringify(prenomData.componentType), !1, interpolationConfig);
        ...
        CompileMetadataResolver = function() {
          ...
          return CompileMetadataResolver.prototype.sanitizeTokenName = function(token) {
            ...
            this._directiveCache.delete(type), this._directiveSummaryCache.delete(type), this._pipeCache.delete(type), this._pipeSummaryCache.delete(type), this._ngModuleOfTypes.delete(type), this._ngModuleCache.clear(), dirMeta && this._directiveNormalizer.clearCacheFor(dirMeta);
          }, CompileMetadataResolver.prototype.clearCache = function() {
            this._directiveCache.clear(), this._directiveSummaryCache.clear(), this._pipeCache.clear(), this._pipeSummaryCache.clear(), this._ngModuleCache.clear(), this._ngModuleOfTypes.clear(), this._directiveNormalizer.clearCache();
            ...
            return value instanceof _angular_core.AnimationStateTransitionMetadata ? new CompileAnimationStateTransitionMetadata(value.stateChangeExpr, this._getAnimationMetadata(value.steps)) : null;
            ...
            if (!this._directiveCache.has(directiveType)) {
              ...
              var moduleUrl = staticTypeModuleUrl(directiveType),
                  createDirectiveMetadata = function(templateMeta) {
                    var changeDetectionStrategy = null,
                        viewProviders = [],
                        entryComponentMetadata = [],
                        selector = dirMeta.selector;
                    if (dirMeta instanceof _angular_core.Component)
                      changeDetectionStrategy = dirMeta.changeDetection, dirMeta.viewProviders && (viewProviders = _this._getProvidersMetadata(dirMeta.viewProviders, entryComponentMetadata, 'viewProviders for "' + stringify(directiveType) + '"')), dirMeta.entryComponents && (entryComponentMetadata = flattenAndDedupeArray(dirMeta.entryComponents).map(function(type) {
                        return _this._getIdentifierMetadata(type, staticTypeModuleUrl(type));
                      }).concat(entryComponentMetadata)), selector || (selector = _this._schemaRegistry.getDefaultComponentElementName());
                    else if (!selector)
                      throw new Error("Directive " + stringify(directiveType) + " has no selector, please add it!");
                    var providers = [];
                    isPresent(dirMeta.providers) && (providers = _this._getProvidersMetadata(dirMeta.providers, entryComponentMetadata, 'providers for "' + stringify(directiveType) + '"'));
                    var queries = [],
                        viewQueries = [];
                    isPresent(dirMeta.queries) && (queries = _this._getQueriesMetadata(dirMeta.queries, !1, directiveType), viewQueries = _this._getQueriesMetadata(dirMeta.queries, !0, directiveType));
                    ...
                    return _this._directiveCache.set(directiveType, meta), _this._directiveSummaryCache.set(directiveType, meta.toSummary()), meta;
                  };
              if (dirMeta instanceof _angular_core.Component) {
                moduleUrl = componentModuleUrl(this._reflector, directiveType, dirMeta), assertArrayOfStrings("styles", dirMeta.styles), assertArrayOfStrings("styleUrls", dirMeta.styleUrls), assertInterpolationSymbols("interpolation", dirMeta.interpolation);
                ...
                if (templateMeta.syncResult)
                  return createDirectiveMetadata(templateMeta.syncResult), null;
                if (isSync)
            ...
            var moduleMeta = this._loadNgModuleMetadata(moduleType, isSync, !1);
            ...
            var ngModule = this._loadNgModuleMetadata(moduleType, isSync, throwIfNotFound),
                loading = ngModule ? Promise.all(ngModule.transitiveModule.loadingPromises) : Promise.resolve(null);
            return {
              ngModule: ngModule,
              loading: loading
            };
            ...
            meta.imports && flattenAndDedupeArray(meta.imports).forEach(function(importedType) {
              ...
              if (isValidType(importedType))
                ...
                importedModuleType = moduleWithProviders.ngModule, moduleWithProviders.providers && providers.push.apply(providers, _this._getProvidersMetadata(moduleWithProviders.providers, entryComponents, "provider for the NgModule '" + stringify(importedModuleType) + "'"));
              }
              ...
              exportedModuleSummary ? exportedModules.push(exportedModuleSummary) : exportedNonModuleIdentifiers.push(_this._getIdentifierMetadata(exportedType, staticTypeModuleUrl(exportedType)));
            });
            ...
            meta.declarations && flattenAndDedupeArray(meta.declarations).forEach(function(declaredType) {
              ...
              if (_this._directiveResolver.isDirective(declaredType)) {
                transitiveModule.directivesSet.add(declaredType), transitiveModule.directives.push(declaredIdentifier), declaredDirectives.push(declaredIdentifier), _this._addTypeToModule(declaredType, moduleType);
                var loadingPromise = _this._loadDirectiveMetadata(declaredType, isSync);
                loadingPromise && transitiveModule.loadingPromises.push(loadingPromise);
              } else {
                ...
                transitiveModule.pipesSet.add(declaredType), transitiveModule.pipes.push(declaredIdentifier), declaredPipes.push(declaredIdentifier), _this._addTypeToModule(declaredType, moduleType), _this._loadPipeMetadata(declaredType);
              }
            });
            var exportedDirectives = [],
                exportedPipes = [];
            if (exportedNonModuleIdentifiers.forEach(function(exportedId) {
            ...
            }), meta.providers && providers.push.apply(providers, this._getProvidersMetadata(meta.providers, entryComponents, "provider for the NgModule '" + stringify(moduleType) + "'")), meta.entryComponents && entryComponents.push.apply(entryComponents, flattenAndDedupeArray(meta.entryComponents).map(function(type) {
            ...
            return entryComponents.push.apply(entryComponents, bootstrapComponents), meta.schemas && schemas.push.apply(schemas, flattenAndDedupeArray(meta.schemas)), (_a = transitiveModule.entryComponents).push.apply(_a, entryComponents), (_b = transitiveModule.providers).push.apply(_b, providers), compileMeta = new CompileNgModuleMetadata({
              ...
              id: meta.id
            }), transitiveModule.modules.push(compileMeta.toInjectorSummary()), this._ngModuleCache.set(moduleType, compileMeta), compileMeta;
            var _a,
                _b;
            ...
            var transitiveModules = getTransitiveImportedModules(importedModules.concat(exportedModules)),
                ...
                transitiveExportedModules = getTransitiveExportedModules(importedModules),
                ...
                loadingPromises = ListWrapper.flatten(transitiveExportedModules.map(function(ngModule) {
            ...
            return type = _angular_core.resolveForwardRef(type), new CompileIdentifierMetadata({
            ...
            this._pipeCache.set(pipeType, meta), this._pipeSummaryCache.set(pipeType, meta.toSummary());
          ...
          }, CompileMetadataResolver.decorators = [{type: _angular_core.Injectable}], CompileMetadataResolver.ctorParameters = [{type: NgModuleResolver}, {type: DirectiveResolver}, {type: PipeResolver}, {type: ElementSchemaRegistry}, {type: DirectiveNormalizer}, {type: ReflectorReader}], CompileMetadataResolver;
        }(),
        _CompileValueConverter = function(_super) {
          function _CompileValueConverter() {
            _super.apply(this, arguments);
          }
          return __extends$17(_CompileValueConverter, _super), _CompileValueConverter.prototype.visitOther = function(value, targetIdentifiers) {
            var identifier;
        ...
        AbstractEmitterVisitor = function() {
          ...
          return AbstractEmitterVisitor.prototype.visitExpressionStmt = function(stmt, ctx) {
            ...
            var value = ast.value;
            return "string" == typeof value ? ctx.print(escapeIdentifier(value, this._escapeDollarInStrings)) : ctx.print("" + value), null;
        ...
        _TsEmitterVisitor = function(_super) {
          ...
          return __extends$18(_TsEmitterVisitor, _super), _TsEmitterVisitor.prototype.visitType = function(t, ctx, defaultType) {
            void 0 === defaultType && (defaultType = "any"), isPresent(t) ? t.visitType(this, ctx) : ctx.print(defaultType);
          }, _TsEmitterVisitor.prototype.visitLiteralExpr = function(ast, ctx) {
            var value = ast.value;
            return isBlank(value) && ast.type != NULL_TYPE ? (ctx.print("(" + value + " as any)"), null) : _super.prototype.visitLiteralExpr.call(this, ast, ctx);
        ...
        __extends$20 = this && this.__extends || function(d, b) {
          function __() {
            this.constructor = d;
          }
          for (var p in b)
        ...
        AbstractJsEmitterVisitor = function(_super) {
          ...
          return __extends$20(AbstractJsEmitterVisitor, _super), AbstractJsEmitterVisitor.prototype.visitDeclareClassStmt = function(stmt, ctx) {
            var _this = this;
        ...
        JitEmitterVisitor = function(_super) {
          ...
          return __extends$19(JitEmitterVisitor, _super), JitEmitterVisitor.prototype.getArgs = function() {
            for (var result = {},
        ...
        RuntimeCompiler = function() {
          ...
          return Object.defineProperty(RuntimeCompiler.prototype, "injector", {
            ...
            var _this = this,
                loadingPromises = [],
                _a = this._metadataResolver.loadNgModuleMetadata(mainModule, isSync),
                ngModule = _a.ngModule,
                loading = _a.loading;
            return loadingPromises.push(loading), ngModule.transitiveModule.modules.forEach(function(localModuleMeta) {
            ...
            ngModule.transitiveModule.modules.forEach(function(localModuleSummary) {
              ...
              localModuleMeta.declaredDirectives.forEach(function(dirIdentifier) {
                ...
                if (_this._compileDirectiveWrapper(dirMeta, localModuleMeta), dirMeta.isComponent && (templates.add(_this._createCompiledTemplate(dirMeta, localModuleMeta)), allComponentFactories)) {
                  var template = _this._createCompiledHostTemplate(dirMeta.type.reference, localModuleMeta);
                  templates.add(template), allComponentFactories.push(template.proxyComponentFactory);
                }
              });
            }), ngModule.transitiveModule.modules.forEach(function(localModuleSummary) {
              var localModuleMeta = _this._metadataResolver.getNgModuleMetadata(localModuleSummary.type.reference);
            ...
            if (!compiledTemplate) {
              ...
              var hostMeta = createHostComponentMeta(compMeta);
              compiledTemplate = new CompiledTemplate((!0), compMeta.selector, compMeta.type, hostMeta, ngModule, [compMeta.type]), this._compiledHostTemplateCache.set(compType, compiledTemplate);
            }
            return compiledTemplate;
          }, RuntimeCompiler.prototype._createCompiledTemplate = function(compMeta, ngModule) {
            var compiledTemplate = this._compiledTemplateCache.get(compMeta.type.reference);
            return compiledTemplate || (assertComponent(compMeta), compiledTemplate = new CompiledTemplate((!1), compMeta.selector, compMeta.type, compMeta, ngModule, ngModule.transitiveModule.directives), this._compiledTemplateCache.set(compMeta.type.reference, compiledTemplate)), compiledTemplate;
            ...
            if (!template.isCompiled) {
              ...
              var parsedAnimations = this._animationParser.parseComponent(compMeta),
                  ...
                  parsedTemplate = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, template.ngModule.schemas, compMeta.type.name),
                  compiledAnimations = this._animationCompiler.compile(compMeta.type.name, parsedAnimations),
                  compileResult = this._viewCompiler.compileComponent(compMeta, parsedTemplate, variable(stylesCompileResult.componentStylesheet.stylesVar), pipes, compiledAnimations);
              compileResult.dependencies.forEach(function(dep) {
                var depTemplate;
                if (dep instanceof ViewClassDependency) {
                  var vfd = dep;
                  depTemplate = _this._assertComponentKnown(vfd.comp.reference, !1), vfd.placeholder.reference = depTemplate.proxyViewClass, vfd.placeholder.name = "View_" + vfd.comp.name;
                } else if (dep instanceof ComponentFactoryDependency) {
                  var cfd = dep;
                  depTemplate = _this._assertComponentKnown(cfd.comp.reference, !0), cfd.placeholder.reference = depTemplate.proxyComponentFactory, cfd.placeholder.name = "compFactory_" + cfd.comp.name;
                } else if (dep instanceof DirectiveWrapperDependency) {
                  var dwd = dep;
              ...
              var viewClass,
                  statements = (_a = stylesCompileResult.componentStylesheet.statements).concat.apply(_a, compiledAnimations.map(function(ca) {
                    return ca.statements;
                  })).concat(compileResult.statements);
              viewClass = this._compilerConfig.useJit ? jitStatements("/" + template.ngModule.type.name + "/" + template.compType.name + "/" + (template.isHost ? "host" : "component") + ".ngfactory.js", statements, compileResult.viewClassVar) : interpretStatements(statements, compileResult.viewClassVar), template.compiled(viewClass);
              var _a;
            }
        ...
        CompiledTemplate = function() {
          ...
          return CompiledTemplate.prototype.compiled = function(viewClass) {
            this._viewClass = viewClass, this.proxyViewClass.prototype = viewClass.prototype, this.isCompiled = !0;
          }, CompiledTemplate;
        }(),
    ...
    var __extends$21 = this && this.__extends || function(d, b) {
      ...
      d.prototype = null === b ? Object.create(b) : (__.prototype = b.prototype, new __);
        ...
        DomElementSchemaRegistry = function(_super) {
          ...
          return __extends$21(DomElementSchemaRegistry, _super), DomElementSchemaRegistry.prototype.hasProperty = function(tagName, propName, schemaMetas) {
